...
class BinarySearchTree{
	...
	public uint GetHeight(TreeNode actRoot) {
		if (Root.Left == null && Root.Right == null)
			return 0;
		uint height = 0;
		if (actRoot.Left != null || actRoot.Right != null) {
			
			uint heightLeft = 0, heightRight = 0;
			
			if (actRoot.Left != null)
				heightLeft = GetHeight(actRoot.Left);
				
			if (actRoot.Right != null)
				heightRight = GetHeight(actRoot.Right);
	...
	public uint Count(TreeNode actRoot) {
		uint count = 0;
		...
		if (actRoot != null)
			count = 1 + Count(actRoot.Left) + Count(actRoot.Right);
		return count;
	}
