...
class SortingAlgos<T> where T : IComparable {
	...
	//Knuth-Shuffle
	public void Shuffle(List<T> arr) {
	...
	private static int PartitionRandom(List<T> arr, int leftIdx, int rightIdx) {
		using (Random rand = new Random()) {	
			T pivot = arr[rand.Next()]; // select the first element in range as the pivot
		}
		int i = leftIdx;
		int	j = rightIdx;
 		while (i < j) {
...
class SortingAlgosMain {

	static void Main() {
		List<int> arr = new List<int>(15);  // array of size 15
		Random r = new Random();
		for (int i = 0; i < 15; i++) {
			arr.Add(r.Next(0, 1001)); //random
			//arr.Add(1000-i); //sorted descending
			//arr.Add(i); //sorted ascending
		}
		SortingAlgos<int>.SelectionSort(arr);
		//SortingAlgos<int>.InsertionSort(arr);
		//SortingAlgos<int>.InsertionSort2(arr);
	    //SortingAlgos<int>.ShellSort(arr);
		//SortingAlgos<int>.BubbleSort(arr);
		//SortingAlgos<int>.BubbleSortNaive(arr);
		//SortingAlgos<int>.MergeSort(arr, 0, arr.Count-1);
		//SortingAlgos<int>.QuickSort(arr, 0, arr.Count-1);
		//SortingAlgos<int>.HeapSort(arr);
		SortingAlgos<int>.Print(arr);
	}
}
