public static void test() {
	...
	Debug.Assert(p.hasNext());
	Debug.Assert(p.next().X == 6);
	Debug.Assert(board.getAt(p) == 0);
	Debug.Assert(board.isFree(p));
	p = new Position(8, 0);
	Debug.Assert(board.getAt(p) == 4);
	Debug.Assert(!board.isFree(p));
	p = new Position(8, 3);
	Debug.Assert(p.hasNext());
	Debug.Assert(p.next().Y == 4);
